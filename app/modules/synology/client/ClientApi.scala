...
class ClientApi @Inject() (wsClient : WSClient,
  ...
  private def loginCall[A](username: String, password: String)(implicit reads: Reads[A]) = retrieve(
    uri = s"/webapi/auth.cgi?api=SYNO.API.Auth&version=2&method=login&account=${UriEncoding.decodePathSegment(username, "utf-8")}&passwd=${UriEncoding.decodePathSegment(password, "utf-8")}&session=DownloadStation&format=cookie"
  )
